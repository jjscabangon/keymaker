...
// Fetch uploaded files (pending, approved, declined) for Business Permits
$sql_pending = "SELECT users.username, users.contact_no, users.email, users_profile.business_permit_url, 
               ...
               WHERE documents.document_type = 'business_permit' AND documents.status = 'pending' 
                     AND users_profile.is_verified = 0";

$sql_verified = "SELECT users.username, users.contact_no, users.email, users_profile.business_permit_url, 
                        documents.document_type, documents.status, documents.approved_by, documents.approved_at, 
                        documents.declined_by, documents.declined_at, users_profile.is_verified, 
                        documents.remarks, documents.document_id 
                 FROM documents
                 INNER JOIN users ON documents.user_id = users.user_id
                 INNER JOIN users_profile ON users.user_id = users_profile.user_id
                 WHERE documents.document_type = 'business_permit' AND documents.status = 'approved'";

$sql_declined = "SELECT users.username, users.contact_no, users.email, users_profile.business_permit_url, 
                        documents.document_type, documents.status, documents.approved_by, documents.approved_at, 
                        documents.declined_by, documents.declined_at, users_profile.is_verified, 
                        documents.remarks, documents.document_id 
                 FROM documents
                 INNER JOIN users ON documents.user_id = users.user_id
                 INNER JOIN users_profile ON users.user_id = users_profile.user_id
                 WHERE documents.document_type = 'business_permit' AND documents.status = 'declined'";
...
<div class="container mt-5">
    ...
    <!-- Navigation Links -->
    <nav class="mb-4">
        <a href="../php/admin_validate_bp.php" class="btn btn-primary">Business Permit</a>
        <a href="../php/admin_validate_pc.php" class="btn btn-secondary">Police Clearance</a>
    </nav>
        ...
        <tbody>
            ...
            if ($pending->num_rows > 0) {
                while ($row = $pending->fetch_assoc()) {
                    $file_path = "../employers/" . $row['business_permit_url'];
                    ...
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['business_permit_url']); ?></td>
                ...
                echo "<tr><td colspan='10'>No pending Business Permits.</td></tr>";
            }
            ?>
        </tbody>
    </table>
        ...
        <table class="table table-bordered table-striped">
            <thead>
                <tr>
                    ...
                    <th>Download</th>
                    <th>View</th>
                    <th>Verify</th> <!-- New column for Verify button -->
                </tr>
            </thead>
            <tbody>
                <?php
                if ($result_verified->num_rows > 0) {
                    ...
                    echo "<tr><td colspan='11'>No approved Business Permits.</td></tr>";
                }
                ?>
            </tbody>
        </table>
    ...
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            if ($result_declined->num_rows > 0) {
                ...
                echo "<tr><td colspan='11'>No declined Business Permits.</td></tr>";
            }
            ?>
        </tbody>
    </table>

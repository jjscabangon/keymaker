...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($rating) || empty($comment)) {
        ...
        header("Location: ../../php/feedbacks/feedback_jobseeker.php?error=Rating and comment are required.&user_id=$viewed_user_id");
        exit();
    }
    ...
    if (mysqli_num_rows($check_user_exists) == 0) {
        ...
        header("Location: ../../php/feedbacks/feedback_jobseeker.php?error=User not found.&user_id=$viewed_user_id");
        exit();
    }

    // Insert feedback into the jobseeker_ratings table
    $insert_feedback = "INSERT INTO jobseeker_ratings (user_id, target_user_id, rating, comment, date_posted) 
                        VALUES ('$user_id', '$viewed_user_id', '$rating', '$comment', NOW())";

    if (mysqli_query($conn, $insert_feedback)) {
        // Redirect after successful feedback
        header("Location: ../../php/view_profile_jobseeker.php?user_id=$viewed_user_id"); // Redirect to profile page
    } else {
        // Redirect with error message
        header("Location: ../../php/feedbacks/feedback_jobseeker.php?error=Error: " . mysqli_error($conn) . "&user_id=$viewed_user_id");
    }
}
?>
...
<div class="wrapper">
    ...
    <!-- Feedback form -->
    <form action="../../php/feedbacks/feedback_jobseeker.php" method="POST">
        <div class="rating">
